...
- v5 protagonist proposals: `protagonist/protagonist-forge-pilgrim-a.png`, `protagonist-coil-ranger-a.png`, and `protagonist-core-bearer-a.png`
- v5 five-gear proposals: fifteen untouched files under `gear/`
- v5 region-background proposals: nine untouched `environment/bg-*.png` files

The first eight provisional machinery selections are rendered in the game for an in-context quality check but have not been copied to `approved/`. Human review selected the switchboard control kit and gauge plus the corrupted blank shard accumulator; those three are now integrated into Godot. The switchboard upgrade rack is integrated as an explicitly replaceable provisional selection because its alternate received only a hold rating. All nine hunt sprites are likewise integrated as first-pass candidates so silhouette and rule readability can be judged in the real map and combat layout before additional generations are spent. The 27 v5 identity candidates are review-only and have not been integrated. The Phase Mantis wing count and the Thermal Titan's humanoid silhouette are explicitly marked for review; weaker creatures will receive three focused variants after playtesting. Every alternate and prompt remains selectable in the local review board, preserving an auditable generation and selection history.
