...
- [First three minutes audit](docs/FIRST_THREE_MINUTES_AUDIT.md)
...
- [High-resolution key-art deliverables](submission/key-art/README.md)
- [Official trailer, score, and beat sheet](submission/trailer/README.md)
...
Audit all 1,440 complete hunt-order and first-boss route permutations:
...
This localhost-only QA mode temporarily unlocks all 33 recovered memories,
...
Preview every enemy, authored scene, HP state and PixelLab defeat animation in
the localhost-only Encounter Lab:

```text
http://127.0.0.1:6681/?encounter_lab=1&mute_test=1
```

The Lab covers all six beasts, both selectable bosses, ARCH SINGULARITY and all
three PRIME CURRENT forms. Use `1`–`5` for the preview states, `T` to verify
dialogue ON/OFF, and `F8` to return to the Lab without touching the save.
`mute_test=1` silences local QA in memory without changing saved audio settings.
...
- VOLT NOMAD uses eighteen original instrumental Suno generations. All nine original enemies and all three PRIME CURRENT forms have exclusive battle music, with separate title, map/tree, endings, artwork, and a short authored victory signal. Exact song URLs, IDs, processing, and runtime assignments are recorded in [`docs/SUNO_MUSIC_BRIEF.md`](docs/SUNO_MUSIC_BRIEF.md).
- The itch.io marketing pass adds original OpenAI-generated high-resolution key art, plus a 60-second gameplay-first Godot trailer scored by a separate Suno instrumental. These marketing-only masters are preserved under [`submission/`](submission/) and do not alter the eighteen-track runtime soundtrack count.
...
VOLT NOMAD v12 contains six direct mechanical-beast battles, always-on AUTO fire, a three-step field manual, five dedicated gear trees with 86 standard nodes and 317 ranks, five permanent simultaneous OVERLIMIT rewrites, PURE COMMAND evolution, eight stolen cores, normal and world-engine endings, a three-form PRIME CURRENT final chapter, 33 bilingual story scenes, authored PixelLab defeat effects, an eight-scene skippable true-ending slideshow, persistent achievements, optional Infinite Mode, atomic saves, playtest telemetry, and mouse/touch/keyboard/remappable-gamepad navigation. The localhost-only Encounter Lab can isolate all 12 enemy presentations without save mutation, and the release audit completes all 1,440 route permutations through true credits and snapshot restoration. Deterministic simulations complete the original normal/full routes in about 7.0/11.5 minutes at five inputs per second; PRIME CURRENT takes about 7.4–12.8 minutes with one OVERLIMIT or 1.5 minutes with all five in the maximum-rank benchmark.
