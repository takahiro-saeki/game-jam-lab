...
func test_charge_clicker_v5() -> void:
	...
	game.close_artwork_viewer()
	game.replay_true_ending_from_artwork()
	check(game.epilogue_open and game.epilogue_return_to_artwork, "the archive can replay the complete true-ending cinematic")
	game.finish_true_epilogue()
	check(game.credits_open and game.credits_context == "artwork_replay", "archive replay continues through the dedicated true credits")
	game.close_credits()
	check(game.artwork_open and not game.title_screen_open, "closing replay credits returns to the artwork archive")
	game.close_artwork_gallery()
