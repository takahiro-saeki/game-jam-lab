...
`npm start` は最新のGodot Web buildも自動で書き出します。「Godot組み合わせ比較」を開くと、原子炉、背景、6セル、GRID WRAITH、エネルギー片の全候補を、固定したボス戦状態の実ゲームUI、発光リング、整数倍表示と重ねて切り替えられます。プレビューモードではゲーム進行を停止し、セーブも読み書きしません。
